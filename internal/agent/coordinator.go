...
func (c *coordinator) Run(ctx context.Context, sessionID string, userPrompt string, attachments ...message.Attachment) (*fantasy.AgentResult, error) {
	...
	// 1. Gather all previously loaded skills from past messages
	var previouslyLoadedSkills []string
	msgs, _ := c.messages.List(ctx, sessionID)
	for _, m := range msgs {
		if sc := m.SkillContext(); sc != nil && len(sc.Skills) > 0 {
			for _, s := range sc.Skills {
				if !slices.Contains(previouslyLoadedSkills, s) {
					previouslyLoadedSkills = append(previouslyLoadedSkills, s)
				}
			}
		}
	}

	// Discover all available skills once and cache them.
	c.ensureSkillsDiscovered()

	// Phase 1: Hardwired keyword-based skill matching.
	// Skills are critical for quality output. Force-match by keyword detection
	// to guarantee relevant skills are always included regardless of embeddings.
	hardwiredSkills := c.matchSkillsByKeyword(userPrompt)

	// Phase 2: Embedding-based skill retrieval (supplementary).
	// Skip embedding for simple greetings or short non-technical requests
	isSimpleGreeting := false
	lowerPrompt := strings.TrimSpace(strings.ToLower(userPrompt))
	if lowerPrompt == "hi" || lowerPrompt == "hello" || lowerPrompt == "hey" || lowerPrompt == "how are you" || lowerPrompt == "how are you?" {
		isSimpleGreeting = true
	}

	var embeddingSkills []*skills.Skill
	if !isSimpleGreeting && c.embeddingService != nil && len(c.discoveredSkills) > 0 {
		if err := c.embeddingService.EmbedSkills(ctx, c.discoveredSkills); err != nil {
			slog.Warn("Failed to embed skills, falling back to keyword-only skill context", "error", err)
		} else {
			relevantSkills, findErr := c.embeddingService.FindRelevantSkills(ctx, userPrompt)
			if findErr != nil {
				slog.Warn("Failed to find relevant skills via embeddings", "error", findErr)
			} else {
				embeddingSkills = relevantSkills
			}
		}
	}

	// Merge hardwired + embedding skills, deduplicate by name.
	newMatchedSkills := mergeSkills(hardwiredSkills, embeddingSkills)

	// 3. Combine previous + new, determine which are actually NEW
	var allSessionSkills []*skills.Skill
	var newSkillsAdded []*skills.Skill

	// First, add all previously loaded skills to allSessionSkills
	for _, s := range c.discoveredSkills {
		displayName := s.Name
		if displayName == "" || displayName == "SKILLNAME" {
			displayName = filepath.Base(s.Path)
		}
		if slices.Contains(previouslyLoadedSkills, displayName) {
			allSessionSkills = append(allSessionSkills, s)
		}
	}

	// Next, check which of the newly matched skills are actually new
	for _, s := range newMatchedSkills {
		displayName := s.Name
		if displayName == "" || displayName == "SKILLNAME" {
			displayName = filepath.Base(s.Path)
		}
		if !slices.Contains(previouslyLoadedSkills, displayName) {
			allSessionSkills = append(allSessionSkills, s)
			newSkillsAdded = append(newSkillsAdded, s)
			activeSkillNames = append(activeSkillNames, displayName) // for UI
			previouslyLoadedSkills = append(previouslyLoadedSkills, displayName)
		}
	}

	if len(allSessionSkills) > 0 {
		skillContext = "<matched_skills>\n"
		for _, s := range allSessionSkills {
			displayName := s.Name
			if displayName == "" || displayName == "SKILLNAME" {
				displayName = filepath.Base(s.Path)
			}
			skillContext += fmt.Sprintf("<skill name=\"%s\">\n%s\n</skill>\n", displayName, s.Instructions)
		}
		skillContext += "</matched_skills>\n\n"
		skillContext += `<skill_usage_instructions>
The following skill(s) were automatically selected as relevant to your task.
Apply their guidance, best practices, and workflows directly.
You have access to a skills folder that enhances your capabilities. These skills were loaded and must be followed strictly.
</skill_usage_instructions>

`
		if len(newSkillsAdded) > 0 {
			slog.Info("Injected NEW skills into context", "skills", activeSkillNames)
		} else {
			slog.Debug("Reused previously loaded skills in context", "count", len(allSessionSkills))
		}
	}
