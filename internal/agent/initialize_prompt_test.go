...
func TestInitializePromptEnforcesDeepCoverageAndSubstantialOutput(t *testing.T) {
	...
	require.Contains(t, built, "unknown location -> `tool_search`")
	require.Contains(t, built, "Batch repeated structured discovery into one call first")
	require.Contains(t, built, "Document length must scale with observed repo size, file count, and domain count")
	require.Contains(t, built, "under roughly 120 lines for a medium or large repository is presumptively incomplete")
	require.Contains(t, built, "Tiny or genuinely simple repos may land around 20-80 lines")
