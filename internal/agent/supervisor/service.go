...
func (s *Service) buildMistakeLogRequiredBody(runtime AgentRuntimeSnapshot, failure agentactivity.Entry) string {
	...
	lines := []string{
		...
		"6. Run a narrow validation probe that would have caught the mistake earlier, and strengthen the same entry if the rule is still weak.",
		"7. Continue the original task only after the file write, durable memory write, and validation loop are complete.",
		"",
		"Failure evidence:",
