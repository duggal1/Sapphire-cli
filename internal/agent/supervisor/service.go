...
func (s *Service) buildMistakeLogRequiredBody(runtime AgentRuntimeSnapshot, failure agentactivity.Entry) string {
	...
	lines := []string{
		...
		"2. Update `MISTAKES.md` at the repository root yourself. Append a new entry only if this is a genuinely new underlying lesson; otherwise strengthen the existing entry instead of creating a near-duplicate.",
		...
		"6. Before validation, persist a focused `improvement_eval` with the task shape, failure signature, exact probe, success criteria, and prevention rule.",
		"7. Run a narrow validation probe that would have caught the mistake earlier, and strengthen the same entry if the rule is still weak.",
		"8. If the probe passes and you used a reusable tactic, persist it with `save_memory` as a `strategy_pattern`.",
		"9. Continue the original task only after the file write, durable memory writes, and validation loop are complete.",
		"",
		"Failure evidence:",
