...
5. Background execution is supported via `background: true`. Use it for long-running work and then `wait` to collect results.
6. Sub-agents cannot spawn sub-agents. Nesting is not allowed.
