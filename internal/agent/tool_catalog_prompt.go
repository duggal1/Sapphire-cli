...
var runtimeToolSummaries = map[string]string{
	...
	"agentic_edit":            "edit 2+ files or batched edits in one structured operation; never call without concrete file_path(s) and edit operations",
	"agentic_fetch":           "fetch and analyze external web sources",
	"agentic_view":            "default repository read tool for one file or many; use it aggressively for repo exploration and broad coverage sweeps",
	"apply_patch":             "apply precise unified-diff patches",
	"bash":                    "absolute last-resort shell fallback for build/test/process work only when structured tools do not fit",
	...
	"edit":                    "legacy single-file edit tool; use single_edit instead when possible",
	"fetch":                   "fetch remote content for inspection",
	"glob":                    "expand one glob pattern per call with optional path or paths roots; never use for content search or multiple patterns",
	"google_search":           "run grounded Google search when available",
	"grep":                    "secondary content search when you explicitly want grep behavior; otherwise prefer tool_search/rg_files/rg routing",
	...
	"ls":                      "inspect directory layout and verify exact paths; not a substitute for search tools",
	...
	"single_edit":             "edit exactly one file with a concrete file_path and precise old/new strings",
	...
	"tool_search":             "first-choice bounded repo locator when the file, symbol, or code region is still unknown",
