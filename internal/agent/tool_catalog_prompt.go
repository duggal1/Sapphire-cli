...
var runtimeToolSummaries = map[string]string{
	...
	"agentic_view":            "primary repo exploration tool; use aggressive parallel multi-file sweeps for almost all non-trivial repo tasks",
	...
	"rg":                      "run real ripgrep content search across the repo",
	"rg_files":                "run ripgrep --files style filename/path search",
	...
	"single_view":             "fallback to read exactly one repository file when one file is truly sufficient",
	...
	"tool_search":             "bounded repo locator for the exact file, symbol, or text region needed before reading",
