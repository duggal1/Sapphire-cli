Last-resort terminal execution tool with automatic background conversion for long-running shell-native tasks.
...
- Concurrent limit: You can execute up to 120 independent `bash` tool requests in parallel per response.
- Good bash use cases: builds, tests, process control, environment inspection, background servers, shell-native tooling with no structured equivalent.
- Bad bash use cases: listing files, searching code, reading files, crafting prompt payload files, or doing anything `ls`, `glob`, `grep`, `single_view`, `agentic_view`, `web_search`, or agent tools already cover.
...
Notes: Use "git commit -am" when possible, don't stage unrelated files, NEVER update config, no -i flags, no empty commits, return empty response.
</git_commits>
