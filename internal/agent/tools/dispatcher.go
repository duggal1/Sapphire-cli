...
// It uses a bounded worker pool to prevent resource exhaustion and
// leverages Go 1.26's optimized sync.Map and runtime improvements.
type Dispatcher struct {
	workers    int
	jobQueue   chan toolJob
	activeJobs sync.Map // Go 1.26 concurrent hash-trie sync.Map
	running    atomic.Bool
	wg         sync.WaitGroup
}
