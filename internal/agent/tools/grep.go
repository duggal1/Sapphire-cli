...
// newRegexCache initializes a new thread-safe cache for compiled regular expressions.
...
const (
	...
	maxGrepFileSize     = 5 * 1024 * 1024 // 5MB
	maxGrepFilesScanned = 5000
)

//go:embed grep.md
var grepDescription []byte

// escapeRegexPattern escapes special characters in a string to make it safe for literal regex matching.
...
func searchWithRipgrep(ctx context.Context, pattern, path, include string) ([]grepMatch, error) {
	...
	if deadline, ok := ctx.Deadline(); ok {
		...
		if timeout > 0 {
			cmd.Args = append(cmd.Args, "--timeout", fmt.Sprintf("%dms", timeout.Milliseconds()))
		}
	}
	cmd.Args = append(cmd.Args, "--max-filesize", fmt.Sprintf("%dM", maxGrepFileSize/(1024*1024)))
...
func searchFilesWithRegex(ctx context.Context, pattern, rootPath, include string) ([]grepMatch, error) {
	...
	err = filepath.Walk(rootPath, func(path string, info os.FileInfo, err error) error {
		// Military-grade safeguard: immediate exit on context cancellation
		if ctx.Err() != nil {
			return ctx.Err()
		}
		...
		filesScanned++
		if filesScanned >= maxGrepFilesScanned {
			return filepath.SkipAll
		}
