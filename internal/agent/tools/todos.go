...
// NewTodosTool creates a tool for managing and updating a structured list of tasks.
	...
	return fantasy.NewAgentTool(
		...
		func(ctx context.Context, params TodosParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			for i, item := range params.Todos {
				...
				switch status {
				case "pending", "pendingstatusid":
					params.Todos[i].Status = "pending"
				case "in_progress", "in-progress":
					params.Todos[i].Status = "in_progress"
				case "completed", "done", "finished", "success":
					params.Todos[i].Status = "completed"
				default:
