...
// SubscribeLSPEvents initializes a subscription to the LSP broker and returns a channel
// for receiving real-time LSP-related events within the provided context.
func SubscribeLSPEvents(ctx context.Context) <-chan pubsub.Event[LSPEvent] {
	return lspBroker.Subscribe(ctx)
}

// GetLSPStates retrieves a point-in-time snapshot of all currently tracked LSP client states.
func GetLSPStates() map[string]LSPClientInfo {
	return lspStates.Copy()
}

// GetLSPState retrieves the current status and client information for a specifically named LSP server.
