...
// findModels identifies all models matching the specified large and small model criteria
// across all configured and enabled providers.
...
// filter applies the model and provider matching logic to determine if a specific model
// and provider pair satisfy the filter criteria.
...
// validateMatches ensures that a model search resulted in exactly one unambiguous match,
// returning an error if no models were found or if the search was ambiguous.
