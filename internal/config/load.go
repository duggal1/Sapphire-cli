...
func Load(workingDir, dataDir string, debug bool) (*Config, error) {
	...
	if err := skills.EnsureProjectSkills(cfg.Options.DataDirectory); err != nil {
		return nil, fmt.Errorf("failed to prepare project skills: %w", err)
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	hasMCPConfig := false
	if c.dataConfigDir != "" {
		hasMCPConfig = c.HasConfigField("mcp")
	}
	if !hasMCPConfig {
		if len(c.MCP) == 0 {
			if err := SeedMCPOnFirstLaunch(context.Background(), c); err != nil {
				slog.Warn("MCP registry seed failed", "error", err)
			}
		}
		if len(c.MCP) == 0 {
			if _, ok := c.MCP["supabase"]; !ok {
				c.MCP["supabase"] = MCPConfig{
					Type: MCPHttp,
	...
	projectSkillsDir := skills.ProjectSkillsDir(c.Options.DataDirectory)
