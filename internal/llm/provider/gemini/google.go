...
func (a *provider) LanguageModel(ctx context.Context, modelID string) (fantasy.LanguageModel, error) {
	...
	if a.options.baseURL != "" || len(a.options.headers) > 0 {
		...
		timeout := 60 * time.Second // Aggressive timeout for fast failure
...
func (g languageModel) prepareParams(call fantasy.Call) (*genai.GenerateContentConfig, []*genai.Content, []fantasy.CallWarning, error) {
	...
	googleSearchEnabled := providerOptions.GoogleSearch != nil && *providerOptions.GoogleSearch
	useFunctionTools := len(call.Tools) > 0
	if googleSearchEnabled && useFunctionTools {
		useFunctionTools = false
		warnings = append(warnings, fantasy.CallWarning{
			Type:    fantasy.CallWarningTypeOther,
			Message: "Google grounding search cannot be combined with Function Calling in the same request. Function calling was disabled for this request.",
		})
	}
	...
	if providerOptions.CodeExecution != nil && *providerOptions.CodeExecution {
		if useFunctionTools {
			warnings = append(warnings, fantasy.CallWarning{
				Type:    fantasy.CallWarningTypeOther,
				Message: "Gemini code execution cannot be combined with function calling in the same request; code execution was skipped",
			})
		} else {
			config.Tools = append(config.Tools, &genai.Tool{
			...
			for _, part := range msg.Content {
				...
				case fantasy.ContentTypeText:
					...
					if currentReasoningMetadata != nil && currentReasoningMetadata.ToolID == "" {
						...
						// Clear the metadata so we do not attach it redundantly to subsequent text parts
						// which would cause "Corrupted thought signature" on the Gemini backend.
				...
				case fantasy.ContentTypeToolCall:
					...
					if metadata, ok := toolCall.ProviderOptions[Name]; ok {
						if googleMetadata, ok := metadata.(*ReasoningMetadata); ok && googleMetadata.Signature != "" {
							appliedSignature = googleMetadata.Signature
						}
					}
					if appliedSignature == "" && currentReasoningMetadata != nil &&
						currentReasoningMetadata.Signature != "" &&
						(currentReasoningMetadata.ToolID == "" || currentReasoningMetadata.ToolID == toolCall.ToolCallID) {
...
func (g *languageModel) Stream(ctx context.Context, call fantasy.Call) (fantasy.StreamResponse, error) {
	...
	return func(yield func(fantasy.StreamPart) bool) {
		...
		var currentPythonToolCallID string
		var hasEmittedGrounding bool
		...
		for resp, err := range chat.SendMessageStream(ctx, depointerSlice(lastMessage.Parts)...) {
			...
			if len(resp.Candidates) > 0 {
				...
				if !hasEmittedGrounding && candidate.GroundingMetadata != nil {
					hasEmittedGrounding = true
					groundingToolCallID := "google_grounding_" + g.providerOptions.toolCallIDFunc()
					query := "Google Search"
					...
					if hasGrounding {
						...
						// Store it in toolCalls so it gets saved in message history
						toolCalls = append(toolCalls, fantasy.ToolCallContent{
							ToolCallID:       groundingToolCallID,
			...
			if len(resp.Candidates) > 0 && resp.Candidates[0].Content != nil {
				for _, part := range resp.Candidates[0].Content.Parts {
					...
					case part.FunctionCall != nil:
						...
						toolCallMeta := fantasy.ProviderMetadata{}
						...
						toolCalls = append(toolCalls, fantasy.ToolCallContent{
							...
							Input:            string(args),
							ProviderExecuted: false,
							ProviderMetadata: toolCallMeta,
						})
					case part.ExecutableCode != nil:
						// Handle Python code execution start
						currentPythonToolCallID = g.providerOptions.toolCallIDFunc()
						if !yield(fantasy.StreamPart{
							Type:         fantasy.StreamPartTypeToolInputStart,
							ID:           currentPythonToolCallID,
							ToolCallName: "python",
						}) {
							return
						}
						if !yield(fantasy.StreamPart{
							Type:  fantasy.StreamPartTypeToolInputDelta,
							ID:    currentPythonToolCallID,
							Delta: part.ExecutableCode.Code,
						}) {
							return
						}
						if !yield(fantasy.StreamPart{
						...
						// Mark as provider executed since it's an internal Gemini tool
						if !yield(fantasy.StreamPart{
							Type:             fantasy.StreamPartTypeToolCall,
							ID:               currentPythonToolCallID,
							ToolCallName:     "python",
							ToolCallInput:    part.ExecutableCode.Code,
							ProviderExecuted: true,
						}) {
							return
						}
					case part.CodeExecutionResult != nil:
						// Handle Python code execution result
						if currentPythonToolCallID != "" {
							if !yield(fantasy.StreamPart{
								Type:         fantasy.StreamPartTypeToolResult,
								ID:           currentPythonToolCallID,
								ToolCallName: "python",
								Delta:        part.CodeExecutionResult.Output,
							}) {
								return
							}
						}
...
func (g languageModel) mapResponse(response *genai.GenerateContentResponse, warnings []fantasy.CallWarning) (*fantasy.Response, error) {
	...
	for _, part := range candidate.Content.Parts {
		...
		case part.FunctionCall != nil:
			...
			toolCallID := cmp.Or(part.FunctionCall.ID, g.providerOptions.toolCallIDFunc())
			foundReasoning := false
			var toolCallProviderMeta fantasy.ProviderMetadata
			if part.ThoughtSignature != nil {
				metadata := &ReasoningMetadata{
					Signature: string(part.ThoughtSignature),
					ToolID:    toolCallID,
				}
				// Store signature on reasoning content for backward compat
				for i := len(content) - 1; i >= 0; i-- {
					c := content[i]
					if c.GetType() == fantasy.ContentTypeReasoning {
						reasoningContent, ok := fantasy.AsContentType[fantasy.ReasoningContent](c)
				...
				// Also store directly on the tool call so it survives round-tripping
				toolCallProviderMeta = fantasy.ProviderMetadata{
					Name: metadata,
				}
			}
			content = append(content, fantasy.ToolCallContent{
				...
				Input:            string(input),
				ProviderExecuted: false,
				ProviderMetadata: toolCallProviderMeta,
			})
			hasToolCalls = true
		case part.ExecutableCode != nil:
			toolCallID := g.providerOptions.toolCallIDFunc()
			content = append(content, fantasy.ToolCallContent{
				ToolCallID:       toolCallID,
				ToolName:         "python",
			...
			hasToolCalls = true
		case part.CodeExecutionResult != nil:
			// Match result with preceding python tool call if possible, or just append
