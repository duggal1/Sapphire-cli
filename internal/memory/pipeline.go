...
type Pipeline struct {
	...
	embedder                  Embedder
	projectRoot               string
	saveArchitecturalDecision func(context.Context, string, ArchitecturalDecision) error
...
func NewPipeline(store *Store, primary MemoryExtractor, secondary MemoryExtractor, embedder Embedder, projectRoot string) *Pipeline {
	active := primary
	if active == nil {
		active = secondary
	}
...
func (p *Pipeline) SetSaveArchitecturalDecision(fn func(context.Context, string, ArchitecturalDecision) error) {
	if p == nil {
		return
	}
	p.saveArchitecturalDecision = fn
}
