...
func settingsHash(opts Settings) string {
	h := xxh3.New()
	fmt.Fprintf(h, "%d-%s-%v-%v-%t",
		opts.Size, opts.Label, opts.LabelColor, opts.GradColors, opts.CycleColors)
...
func New(opts Settings) *Anim {
	...
	if len(opts.GradColors) < 2 {
		opts.GradColors = []color.Color{defaultGradColorA, defaultGradColorB}
	}
