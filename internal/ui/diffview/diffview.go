...
func (dv *DiffView) hunkLineFor(h *udiff.Hunk) string {
	...
	var b strings.Builder
	b.WriteString(dv.style.HunkLine.Base.Render("  @@ "))
	b.WriteString(dv.style.HunkLine.Minus.Render(fmt.Sprintf("-%d,%d", h.FromLine, beforeShownLines)))
	b.WriteString(dv.style.HunkLine.Base.Render(" "))
	b.WriteString(dv.style.HunkLine.Plus.Render(fmt.Sprintf("+%d,%d", h.ToLine, afterShownLines)))
	b.WriteString(dv.style.HunkLine.Base.Render(" @@ "))
	return b.String()
}
